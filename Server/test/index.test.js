...
describe('Test de Rutas', () => {
    ...
    describe("GET /rickandmorty/login", () => {
        it('La informacion del login es true', async () => {
            const response = await (await agent.get('/rickandmorty/login?email=victorseva123@gmail.com&password=1234Abcd')).body;
            expect(response.access).toEqual(true);
        })

        it('La informacion del login es false', async () => {
            const response = (await agent.get('/rickandmorty/login?email=victorseva123com&password=bcd')).body;
